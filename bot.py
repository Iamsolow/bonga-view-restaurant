...
from database import init_db, add_menu_item, get_menu_items, add_expense_item, get_expense_items, record_transaction, get_detailed_report, get_recent_transactions, delete_transaction, is_admin, is_staff, add_user
...
def main_menu(user_id):
    if is_admin(user_id):
        kb = [[KeyboardButton(text="💰 ገቢ መመዝገቢያ"), KeyboardButton(text="💸 ወጪ መመዝገቢያ")],[KeyboardButton(text="📦 ስቶር/ዝርዝር"), KeyboardButton(text="📊 ሪፖርት")],[KeyboardButton(text="🗑️ ሂሳብ ሰርዝ"), KeyboardButton(text="👤 ሰራተኛ ጨምር")]]
    else:
        kb = [[KeyboardButton(text="💰 ገቢ መመዝገቢያ"), KeyboardButton(text="💸 ወጪ መመዝገቢያ")]]
...
async def start(message: types.Message):
    ...
    if is_staff(message.from_user.id):
        await message.answer("እንኳን ወደ ሬስቶራንት ሂሳብ መቆጣጠሪያ ቦት በሰላም መጡ!", reply_markup=main_menu(message.from_user.id))
    else:
        await message.answer(f"ይቅርታ፣ እርስዎ ገና አልተመዘገቡም። የእርስዎ ID: {message.from_user.id} ነው። እባክዎን አድሚኑን እንዲመዘግብዎት ይጠይቁ።")
...
async def process_income_qty(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"✅ የ{data['item']} ሽያጭ ተመዝግቧል (ጠቅላላ: {data['amount'] * qty} ብር)።", reply_markup=main_menu(message.from_user.id))
        await state.clear()
    except ValueError: await message.answer("ቁጥር ብቻ ያስገቡ!")
...
async def add_menu_price(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"✅ {data['name']} ተመዝግቧል። የሽያጭ ዋጋውን ያስገቡ፦")
...
async def process_exp_qty(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"✅ የ{data['item']} ወጪ ተመዝግቧል።", reply_markup=main_menu(message.from_user.id))
        await state.clear()
    except ValueError: await message.answer("ቁጥር ብቻ ያስገቡ!")
